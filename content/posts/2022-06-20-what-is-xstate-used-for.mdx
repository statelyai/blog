...
XState can be used in any frontend application to manage state. It has first-class integrations for [React](TODO), [Vue](TODO), [Svelte](TODO), as well as an upcoming [Solid](TODO) integration. It also works well in [Angular](TODO) without any integrations needed.
...
You can also use XState to manage global state in your apps. A common pattern is the [Flux](TODO) architecture, where you dispatch events to a single top-level store. Parts of your app can subscribe to updates from that store, using [selectors](TODO).

XState can replace global state managers like [Redux](TODO) or [Vuex](TODO), with one major benefit - your global state can be visualised and [visually edited](TODO). XState gives you the ability to create a global store, dispatch events to it, and subscribe to only the pieces that matter. See our specific instructions in [React](TODO) for more info.

## Native/extensions

XState is also extremely useful in [React Native](TODO) or [Electron](TODO) apps, Browser extensions and IDE extensions. Since XState doesn't use any browser API's, you can use it anywhere JavaScript runs.
...
You can use XState in [Node.js](TODO) or [Deno](TODO) to build [lambda functions](TODO) or persistent servers.

For lambda functions, XState exposes a function called [`waitFor`](TODO), which allows you to wait for a state machine to be in a certain state. This allows you to use XState inside `async` functions with ease.

To learn more about XState in the backend, see our [recent video](TODO) introducing the topic.

## Scripting and CLI's

XState can be used when running scripts or CLI's to co-ordinate long-running processes.

The most famous example of this in the wild is the frontend framework [Gatsby](TODO). They use XState in their build process and development server to co-ordinate file-system changes and parallelize multiple processes. This means their entire build system is visualisable - an incredibly boon for such a complex process.

## Summary

XState works anywhere JS runs - and folks are using it in the wild for _all sorts_ or use cases. Any time you're building something that feels remotely complex, you can use XState to simplify it and enable powerful visual tooling.
