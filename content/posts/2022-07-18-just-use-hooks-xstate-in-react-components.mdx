...
For the `Toggler` component’s `onChange` handler, we set its value to be the `toggle` function. Since that is already wrapping XState’s `send('TOGGLE')` method, we don’t even need to wrap this in an anonymous function. It all just works as is, in a tidy functional style.
