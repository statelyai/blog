...
Here is a more complete [CodeSandbox example for a single callback param](https://codesandbox.io/s/usetogglemachine-callback-params-example-1-fujl6m?file=/src/useToggleMachine.ts).
...
### Passing multiple callback params
...
Here is a [CodeSandbox example for passing multiple callback params](https://codesandbox.io/s/usetogglemachine-multiple-callback-params-example-2-m6pqoq?file=/src/Toggler.tsx).
...
const createToggleMachine = (initialActive: boolean) =>
  createMachine({
    ...
    states: {
      inactive: {
        on: {
          TOGGLE: "togglingActive",
        },
      },
      active: {
        on: {
          TOGGLE: "togglingInactive",
        },
      },
      togglingActive: {
        invoke: {
